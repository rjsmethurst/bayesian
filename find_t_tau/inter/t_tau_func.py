...
def sample(ndim, nwalkers, nsteps, start, w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps):
    ...
    fig.savefig('triangle_t_tau_inter_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
...
def walker_plot(samples, nwalkers, limit):
    ...
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/inter/walkers_steps_inter_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
...
def walker_steps(samples, nwalkers, limit):
    ...
    ax1.imshow(ur, origin='lower', aspect='auto', alpha = 0.3, extent=[0, 13.8, 0, 3])
    ax2.imshow(ur, origin='lower', aspect='auto', alpha = 0.3, extent=[0, 13.8, 0, 3])
    ax3.imshow(nuv, origin='lower', aspect='auto', alpha = 0.3, extent=[0, 13.8, 0, 3])
    ax4.imshow(nuv, origin='lower', aspect='auto', alpha = 0.3, extent=[0, 13.8, 0, 3])
    for n in range(len(s)):
    ...
    ax4.set_ylabel(r'$\tau_{disc}$')
    P.tight_layout()
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/inter/walkers_2d_steps_inter_'+str(nwalkers)+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
    fig.savefig(save_fig)
    return fig
