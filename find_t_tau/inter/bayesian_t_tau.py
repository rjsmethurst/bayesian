...
#samples, fig, samples_save = sample(4, nwalkers, nsteps, start, w, inter[:,0], inter[:,4], inter[:, 1], inter[:, 5], age, inter[:,3], inter[:,2])
#elap = (time.time() - start_time)/60
#print 'Minutes taken for '+str(len(samples)/nwalkers
#                               )+' steps and '+str(nwalkers)+' walkers', elap

samples = N.load('samples_inter_60000_63955_05_01_24_03_14.npy')
...
fig_s = triangle.corner(samples[:,0:2], labels = [r'$ t_{smooth}$', r'$ \tau_{smooth}$'], extents =[(0, 13.807108309208775),(0, 3.0)])
fig_s.savefig('triangle_t_tau_smooth_inter_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = triangle.corner(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'], extents =[(0, 13.807108309208775),(0, 3.0)])
fig_d.savefig('triangle_t_tau_disc_inter_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')

fig_samp = walker_plot(unburnt_samples, nwalkers, 300)

fig_2 = walker_steps(samples, nwalkers, 300)
