...
def sample(ndim, nwalkers, w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps):
    ...
    pos = [[13.0, 0.001, 13.0, 0.001] + 1e-4*N.random.randn(ndim) for i in range(nwalkers)]
    ...
    samples_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/gv/samples_gv_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy'
...
def walker_steps(samples, nwalkers, limit):
    ...
    ax4.set_ylabel(r'$\tau_{disc}$')
    P.tight_layout()
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/gv/walkers_2d_steps_gv_'+str(nwalkers)+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
    fig.savefig(save_fig)
    return fig
