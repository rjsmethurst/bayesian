...
#gv = gvf[gvf[:,9]==1]
#gv_nd = gv[gv[:,3] < 0.8]
#gv_dnd = gv_nd[gv_nd[:,3] > 0.2]
#gv_ns = gv[gv[:,2] < 0.8]
#gv_dns = gv_ns[gv_ns[:,2] > 0.2]
#gv_nc = N.append(gv_dnd, gv_dns, axis=0)
#print len(gv)
...
#print len(blue_c)
#
#age_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/gv/not_clean/age_gv_not_clean.npy'
#age_path = os.path.exists(age_save)
#cosmo = FlatLambdaCDM(H0 = 71.0, Om0 = 0.26)
#if age_path ==False:
#    age = N.array(cosmo.age(gv_nc[:,6]))
#    N.save(age_save, age)
#else:
#    age = N.load(age_save)
#print len(age)
...
#samples, fig = sample(4, nwalkers, nsteps, start, w, gv_nc[:,0], gv_nc[:,4], gv_nc[:, 1], gv_nc[:, 5], age, gv_nc[:,3], gv_nc[:,2])
#elap = (time.time() - start_time)/60
#print 'Minutes taken for '+str(len(samples)/nwalkers
#                               )+' steps and '+str(nwalkers)+' walkers', elap

samples = N.load('samples_gv_not_clean_15000_29224_09_55_10_04_14.npy')
...
fig_s.savefig('triangle_t_tau_smooth_gv_not_clean_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = corner_plot(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'])
fig_d.savefig('triangle_t_tau_disc_gv_not_clean_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
