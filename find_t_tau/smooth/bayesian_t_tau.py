...
samples, fig = sample(4, nwalkers, w, smooth[:,0], smooth[:,4], smooth[:, 1], smooth[:, 5], age, smooth[:,3], smooth[:,2])
...
fig_s.savefig('triangle_t_tau_smooth_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = triangle.corner(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'])
fig_d.savefig('triangle_t_tau_disc_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')

fig_samp = walker_plot(samples, nwalkers, 150)
fig_2 = walker_steps(samples, nwalkers, 150)
