...
samples, fig = sample(4, nwalkers, nsteps, start, w, disc[:,0], disc[:,4], disc[:, 1], disc[:, 5], age, disc[:,3], disc[:,2])
elap = (time.time() - start_time)/60
print 'Minutes taken for '+str(len(samples)/nwalkers
                               )+' steps and '+str(nwalkers)+' walkers', elap

#samples = N.load('samples_disc_20000_51662_00_27_12_03_14.npy')
...
fig_s.savefig('triangle_t_tau_smooth_disc_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = corner_plot(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'])
fig_d.savefig('triangle_t_tau_disc_disc_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
