...
def sample(ndim, nwalkers, nsteps, start, w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps):
    ...
    samples_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/mergers/samples_mergers_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy'
    N.save(samples_save, samples)
    fig = triangle.corner(samples, labels=[r'$ t_{merger} $', r'$ \tau_{merger} $', r'$ t_{no merger} $', r'$ \tau_{no merger}$'])
    fig.savefig('triangle_t_tau_mergers_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
...
def walker_plot(samples, nwalkers, limit):
    ...
    ax1.set_ylabel(r'$t_{bar}$')
    ax2.set_ylabel(r'$\tau_{bar}$')
    ax3.set_ylabel(r'$t_{no bar}$')
    ax4.set_ylabel(r'$\tau_{no bar}$')
    P.subplots_adjust(hspace=0.1)
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/mergers/walkers_steps_mergers_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
...
def walker_steps(samples, nwalkers, limit):
    ...
    ax4.set_ylabel(r'$\tau_{no bar}$')
    P.tight_layout()
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/mergers/walkers_2d_steps_mergers_'+str(nwalkers)+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
    fig.savefig(save_fig)
    return fig
