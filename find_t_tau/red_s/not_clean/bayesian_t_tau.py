...
#red_s_ns = red_s[red_s[:,2] < 0.8]
#red_s_d = red_s[red_s[:,3] >= 0.8]
#red_s_nd = red_s[red_s[:,3] < 0.8]
#red_s_clean = N.append(red_s_s, red_s_d, axis=0)
#red_s_nc = N.append(red_s_ns, red_s_nd, axis=0)
#print len(red_s_nc)
#print len(red_s)
#blue_c = colours[colours[:,0] < colours[:,11]]
#print len(blue_c)
#
#age_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/red_s/not_clean/age_red_s_nc.npy'
#age_path = os.path.exists(age_save)
#cosmo = FlatLambdaCDM(H0 = 71.0, Om0 = 0.26)
#if age_path ==False:
#    age = N.array(cosmo.age(red_s_nc[:,6]))
#    N.save(age_save, age)
#else:
#    age = N.load(age_save)
#print len(age)
...
#samples, fig, samples_save = sample(4, nwalkers, nsteps, start, w, red_s_nc[:,0], red_s_nc[:,4], red_s_nc[:, 1], red_s_nc[:, 5], age, red_s_nc[:,3], red_s_nc[:,2])
#elap = (time.time() - start_time)/60
#print 'Minutes taken for '+str(len(samples)/nwalkers)+' steps and '+str(nwalkers)+' walkers', elap

samples = N.load('samples_red_s_not_clean_15000_46182_20_45_10_04_14.npy')
...
fig_s.savefig('triangle_t_tau_smooth_red_s_nc_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
fig_d = corner_plot(samples[:,2:4], labels = [r'$t_{disc}$', r'$\tau_{disc}$'])
fig_d.savefig('triangle_t_tau_disc_red_s_nc_'+str(len(samples))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
