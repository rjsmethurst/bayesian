...
def lnlike_one(theta, ur, sigma_ur, nuvu, sigma_nuvu, age):
    ...
    #inv_sigma = 1./((sigma_ur**2)*(sigma_nuvu**2)*(N.pi*2)**2)**0.5
    inv_sigma = 1./((sigma_ur**2)*(2*N.pi))**0.5
    #return N.log(inv_sigma)-0.5*((ur-pred_ur)**2/sigma_ur**2) #- 0.5*((nuvu-pred_nuvu)**2/sigma_nuvu**2)
    #print 'returned lnlike_one', -0.5*((ur-pred_ur)**2/sigma_ur**2)
    return -0.5*((ur-pred_ur)**2/sigma_ur**2)
...
def lnlike(theta, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps):
    ...
    S = N.log(ps) + s
    #print 'S', S
    #print 'logaddexp', N.logaddexp(D, S)
    #print 'lnlike', N.sum(N.logaddexp(D, S))
    return N.sum(N.logaddexp(D, S))
    #return N.sum(N.log(pd*N.exp(d) + ps*N.exp(s)))
...
def sample(ndim, nwalkers, w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps):
    ...
    pos = [[9.0, 1.25, 9.0, 1.25] + 1e-4*N.random.randn(ndim) for i in range(nwalkers)]
    ...
    samples_save = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/blue_cloud/samples_blue_c_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.npy'
    N.save(samples_save, samples)
    fig = triangle.corner(samples, labels=[r'$ t_{smooth} $', r'$ \tau_{smooth} $', r'$ t_{disc} $', r'$ \tau_{disc}$'])
    fig.savefig('triangle_t_tau_blue_c_'+str(len(samples))+'_'+str(len(age))+'_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf')
    return samples, fig
...
#    f = open('chain.dat','w')
#    f.close()
#    pos = [w[0:4] + 1e-4*N.random.randn(ndim) for i in range(nwalkers)]
#    sampler = emcee.EnsembleSampler(nwalkers, ndim, lnprob, args=(w, ur, sigma_ur, nuvu, sigma_nuvu, age, pd, ps))
#    for result in sampler.sample(pos, iterations=10, storechain=False):
#        position = result[0]
#        f =open('chain.dat','a')
#        for k in range(position.shape[0]):
#        f.write("{0:4d} {1:s}\n".format(k, " ".join(position[k])))
#            f.close()
#            if sampler.iterations %50 == 0:
#                print 'Iteration', sampler.iterations
#                print 'Current Params', position[k]
#                print 'Elapsed time in minutes', (time.time() - start_time)/60
#            print N.shape(sampler.chain)
#            samples = sampler.chain[:,:,:].reshape((-1,ndim))
#            print N.shape(samples)
#            fig = position
#            print N.shape(position)
    #fig = triangle.corner(samples, labels=[r'$ t_{smooth} $', '$ \tau_{smooth} $', '$ t_{disc} $', '$ \tau_{disc}$'])
    #fig.savefig('triangle_t_tau_gv100.png')
#    return samples, fig
...
def walker_plot(samples, nwalkers, limit):
    ...
    save_fig = '/Users/becky/Projects/Green-Valley-Project/bayesian/find_t_tau/blue_cloud/walkers_steps_blue_c_'+str(time.strftime('%H_%M_%d_%m_%y'))+'.pdf'
